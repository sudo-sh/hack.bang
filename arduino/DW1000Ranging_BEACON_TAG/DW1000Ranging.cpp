...
volatile short DW1000RangingClass::_networkDevicesNumber=0;
...
void DW1000RangingClass::configureNetwork(unsigned int deviceAddress, unsigned int networkId, const byte mode[],byte channel){
    // general configuration
    DW1000.newConfiguration();
    DW1000.setDefaults();
    DW1000.setDeviceAddress(deviceAddress);
    DW1000.setNetworkId(networkId);
    DW1000.enableMode(mode,channel);
...
void DW1000RangingClass::startAsAnchor(char address[],  const byte mode[],byte channel){
    //save the address
...
void DW1000RangingClass::startAsTag(char address[],  const byte mode[],byte channel){
    //save the address
    DW1000.convertToByte(address, _currentAddress);
    //write the address on the DW1000 chip
    DW1000.setEUI(address);
...
//
//void DW1000RangingClass::getDataMobile(){
//  
//
////readDataMobile Varaible
//byte reqchar[10];
//
//
//   int temp_read=0;
//
//        for(temp_read=90;temp_read<100;temp_read++)//Now the length of data taken is 10 bytes
//        {
//          reqchar[90-temp_read]=data[temp_read];
//        }
//        int i=0;
//     for(i=0;i<10;i++)//The length of the data taken as extra is 10 bytes
//  {
//    Serial.print(char(reqchar[i]));Serial.print("  ");
//  }
//  Serial.println();
//}

void DW1000RangingClass::loop(int flag){
    ...
    if(_receivedAck){
        ...
        //Now we read the sent from the mobiles Data
       /*
        //Now we read the sent Data.
        int temp_read=0;
        Serial.print("Extra Data ") ;
        for(temp_read=91;temp_read<100;temp_read++)
        {
          Serial.print("  ,");Serial.print(char(data[temp_read]));
        }
        Serial.println();
        */
