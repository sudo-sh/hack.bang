...
void DW1000Class::enableMode(const byte mode[],byte channel) {
	setDataRate(mode[0]);
...
void DW1000Class::commitConfiguration() {
	...
	writeValueToBytes(antennaDelayBytes, 16505, LEN_STAMP);
...
//        //setFrameFilterAllowAcknowledgement(true);
		...
		enableMode(MODE_LONGDATA_RANGE_LOWPOWER,CHANNEL_5);
	}
}
...
void DW1000Class::setData(byte data[], unsigned int n) {
  ...
  //Now I will send 10 byte of Data of my preference
  //We want this data to be used since this is sent by the Mobiles and contain the Information regarding the 
  //Speed
  //Braking Conditions
  //Number of Vehicals at the Back
  //GPS Data 
  //and Misc.

  //The 110 limit is present in the LEN_Data of the DW1000Ranging.h
//  int temp_fill=0;
//  for(temp_fill=91;temp_fill<110;temp_fill++) //The 110 is the maximum length of the data that can be sent using the Decawave Sensors 
//  
//  {
//    data[temp_fill]=66;
//  }
...
void DW1000Class::getfilldata(byte inputdata[],int i)
{
  //i should be 19 bytes if the LEN_Data is not changed.
}


//#################################################################################################################################

//This function is used to send the written data through the Beacon to the Mobiles

void DW1000Class::sendDataMobiles(byte data[],byte filldata[])//Now filldata needs to be of 19 bytes only change LEN_DATA to increase
{
  //The filldata is use to take the entry from the user and fill it to the data_packet sent
  //i is the number of data to be filled the maximum data that can be accomodated

  //We can send from 91 to 109 bytes =19 bytes till now We can increase this.
